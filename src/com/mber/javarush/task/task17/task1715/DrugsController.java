package com.mber.javarush.task.task17.task1715;
...
public class DrugsController {
    ...
    public synchronized void buy(Drug drug, int count) {
        ...
        if (!allDrugs.containsKey(drug)) System.out.println("Нет в наличии");
        Integer currentCount = allDrugs.get(drug);
        if (currentCount < count)
            System.out.printf("%s хочет %s %d шт. В наличии - %d%n"
                    , name, drug.getName(), count, currentCount);
        else {
            allDrugs.put(drug, (currentCount - count));
            System.out.printf("%s купил(а) %s %d шт. Осталось - %d%n"
